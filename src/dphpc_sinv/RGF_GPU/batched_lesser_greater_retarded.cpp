...
void rgf_lesser_greater_retarded_for(
    unsigned int blocksize,
    unsigned int matrix_size,
    unsigned int batch_size,
    complex_h **system_matrix_diagblk_h,
    ...
    complex_h **retarded_inv_lowerblk_h)
{
    ...
    for(int i = 0; i < 2; i++){
        cudaErrchk(cudaMalloc((void**)&retarded_inv_diagblk_small_d[i], blocksize * blocksize * sizeof(complex_d)));
    ...
    for(unsigned int batch = 0; batch < batch_size; batch++){
        ...
        cudaErrchk(cudaStreamWaitEvent(stream[stream_memunload], schur_inverted[0]));
        // 0. Inverse of the first block
        cudaErrchk(cudaMemcpyAsync(retarded_inv_diagblk_h[0] + batch * blocksize * blocksize, retarded_inv_diagblk_d,
        ...
        for (unsigned int i = 1; i < n_blocks; ++i) {
            ...
            cublasErrchk(cublasZgemm(
                ...
                retarded_inv_diagblk_d, blocksize,
                &beta,
                retarded_inv_diagblk_small_d[stream_compute], blocksize));
            ...
            // wait to unload_lesser_greater_diag for the finish of computations
            cudaErrchk(cudaStreamWaitEvent(stream[stream_memunload], schur_inverted[i]));
            cudaErrchk(cudaMemcpyAsync(retarded_inv_diagblk_h[i] + batch * blocksize * blocksize,
                        retarded_inv_diagblk_d,
                        blocksize * blocksize * sizeof(complex_d), cudaMemcpyDeviceToHost, stream[stream_memunload]));
        ...
        cudaErrchk(cudaMemcpyAsync(retarded_inv_diagblk_small_d[stream_memload_before],
                    reinterpret_cast<const complex_d*>(retarded_inv_diagblk_h[n_blocks-2] + batch*blocksize*blocksize),
                    blocksize * blocksize * sizeof(complex_d), cudaMemcpyHostToDevice, stream[stream_memload_before]));
        cudaErrchk(cudaMemcpyAsync(lesser_inv_diagblk_small_d[stream_memload_before],
        ...
        for(int i = n_blocks-2; i >= 0; --i){
            ...
            if(i > 0){
                ...
                cudaErrchk(cudaMemcpyAsync(system_matrix_lowerblk_d[stream_memload],
                            ...
                            blocksize * blocksize * sizeof(complex_d), cudaMemcpyHostToDevice, stream[stream_memload]));
                cudaErrchk(cudaMemcpyAsync(retarded_inv_diagblk_small_d[stream_memload],
                            reinterpret_cast<const complex_d*>(retarded_inv_diagblk_h[i-1] + batch*blocksize*blocksize),
                            blocksize * blocksize * sizeof(complex_d), cudaMemcpyHostToDevice, stream[stream_memload]));
            ...
            cudaErrchk(cudaMemcpyAsync(retarded_inv_diagblk_h[i] + batch * blocksize * blocksize,
                        retarded_inv_diagblk_d,
                        blocksize * blocksize * sizeof(complex_d), cudaMemcpyDeviceToHost, stream[stream_memunload]));
            cudaErrchk(cudaMemcpyAsync(retarded_inv_upperblk_h[i] + batch * blocksize * blocksize,
                        retarded_inv_upperblk_d,
                        blocksize * blocksize * sizeof(complex_d), cudaMemcpyDeviceToHost, stream[stream_memunload]));       
            cudaErrchk(cudaMemcpyAsync(retarded_inv_lowerblk_h[i] + batch * blocksize * blocksize,
                        retarded_inv_lowerblk_d,
                        blocksize * blocksize * sizeof(complex_d), cudaMemcpyDeviceToHost, stream[stream_memunload]));
    ...
    complex_h **retarded_inv_lowerblk_h)
{
    ...
    complex_d* identity_d = NULL;
    complex_h* identity_h = (complex_h*)calloc(batch_size * blocksize * blocksize, sizeof(complex_h));

    cudaErrchk(cudaMalloc((void**)&identity_d, batch_size * blocksize * blocksize * sizeof(complex_d)));

    // create identity matrix
    for(unsigned int batch = 0; batch < batch_size; batch++){
        for(unsigned int i = 0; i < blocksize; i++){
            identity_h[i + i*blocksize + batch * blocksize * blocksize] = 1.0;
        }
    }
    ...
    cudaErrchk(cudaMemcpyAsync(retarded_inv_diagblk_d, identity_d,
                batch_size * blocksize * blocksize * sizeof(complex_d),
                cudaMemcpyDeviceToDevice, stream[stream_compute]));

    complex_d *buffer_d = NULL;
    int bufferSize = 0;
    cusolverErrchk(cusolverDnZgetrf_bufferSize(cusolver_handle[stream_compute], blocksize, blocksize,
                                            (complex_d *)system_matrix_diagblk_d[stream_compute],
                                              blocksize, &bufferSize));
    cudaErrchk(cudaMalloc((void**)&buffer_d, sizeof(complex_d) * bufferSize));    
    ...
    for(unsigned int batch = 0; batch < batch_size; batch++){
        cusolverErrchk(cusolverDnZgetrf(
                cusolver_handle[stream_compute],
                blocksize,
                blocksize,
                system_matrix_diagblk_d[stream_compute] + batch * blocksize * blocksize,
                blocksize,
                buffer_d,
                ipiv_d + batch * blocksize,
                info_d + batch));
        // inversion
        cusolverErrchk(cusolverDnZgetrs(
                    cusolver_handle[stream_compute],
                    CUBLAS_OP_N,
                    blocksize,
                    blocksize,
                    ...
                    blocksize,
                    ipiv_d + batch * blocksize,
                    retarded_inv_diagblk_d + batch * blocksize * blocksize,
                    blocksize,
                    info_d + batch)); 
    }
    ...
    for(unsigned int batch = 0; batch < batch_size; batch++){
        cublasErrchk(cublasZgemm(
            ...
            blocksize, blocksize, blocksize,
            &alpha,
            self_energy_lesser_diagblk_d[stream_compute] + batch * blocksize * blocksize,
            blocksize,
            retarded_inv_diagblk_d + batch * blocksize * blocksize,
            blocksize,
            &beta,
            self_energy_lesser_upperblk_d[stream_compute] + batch * blocksize * blocksize,
            blocksize));
        ...
        cublasErrchk(cublasZgemm(
            ...
            blocksize, blocksize, blocksize,
            &alpha,
            self_energy_greater_diagblk_d[stream_compute] + batch * blocksize * blocksize,
            blocksize,
            retarded_inv_diagblk_d + batch * blocksize * blocksize,
            blocksize,
            &beta,
            self_energy_greater_upperblk_d[stream_compute] + batch * blocksize * blocksize,
            blocksize));
        ...
        cublasErrchk(cublasZgemm(
            ...
            blocksize,
            &beta,
            lesser_inv_diagblk_d + batch * blocksize * blocksize,
            blocksize));
        ...
        cublasErrchk(cublasZgemm(
            ...
            blocksize,
            &beta,
            greater_inv_diagblk_d + batch * blocksize * blocksize,
            blocksize));
    }
    cudaErrchk(cudaEventRecord(lesser_greater_calculated[0], stream[stream_compute]));
    ...
    for (unsigned int i = 1; i < n_blocks; ++i) {
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize, blocksize, blocksize,
                &alpha,
                system_matrix_lowerblk_d[stream_compute] + batch * blocksize * blocksize,
                blocksize,
                retarded_inv_diagblk_d + batch * blocksize * blocksize,
                blocksize,
                &beta,
                retarded_inv_diagblk_small_d[stream_compute] + batch * blocksize * blocksize,
                blocksize));
        }
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                retarded_inv_diagblk_small_d[stream_compute] + batch * blocksize * blocksize,
                blocksize,
                system_matrix_upperblk_d[stream_compute] + batch * blocksize * blocksize,
                blocksize,
                &beta,
                system_matrix_diagblk_d[stream_compute] + batch * blocksize * blocksize,
                blocksize));
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                lesser_inv_upperblk_d + batch * blocksize * blocksize,
                blocksize));
            cublasErrchk(cublasZgemm(
                cublas_handle[stream_compute],
                CUBLAS_OP_N, CUBLAS_OP_N,
                ...
                blocksize,
                &beta,
                greater_inv_upperblk_d + batch * blocksize * blocksize,
                blocksize));
        }
    ...
    for(int i = n_blocks-2; i >= 0; --i){
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                buf4_lesser_d + batch * blocksize * blocksize,
                blocksize));
            cublasErrchk(cublasZgemm(
                cublas_handle[stream_compute],
                CUBLAS_OP_N, CUBLAS_OP_C,
                ...
                blocksize,
                &beta,
                buf4_greater_d + batch * blocksize * blocksize,
                blocksize));
        }
        ...
        //     System_matrix[i_plus_one_, i_]) 
        alpha = make_cuDoubleComplex(1.0, 0.0);
        complex_d *buf2_d = self_energy_lesser_diagblk_d[stream_compute];
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                buf2_d + batch * blocksize * blocksize,
                blocksize));
        }
        // buf1 = (g_retarded[i_, i_] @
        //     System_matrix[i_, i_plus_one_])
        // self_energy_greater_diagblk_d[stream_compute] is not overwritten by memcpy
        // since it is only used in the forward pass
        complex_d *buf1_d = self_energy_greater_diagblk_d[stream_compute];
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                cublas_handle[stream_compute],
                CUBLAS_OP_N, CUBLAS_OP_N,
                ...
                blocksize,
                &beta,
                buf1_d + batch * blocksize * blocksize,
                blocksize));
        }
        // buf7 = - buf2 @
        //     g_retarded[i_, i_]      
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                buf7_d + batch * blocksize * blocksize,
                blocksize));
        }
        // G_tmp   =  g_retarded[i_, i_] - (buf1 @
        //                                     buf7)
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                retarded_inv_upperblk_d + batch * blocksize * blocksize,
                blocksize));
        }
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                buf2_d + batch * blocksize * blocksize,
                blocksize,
                lesser_inv_diagblk_small_d[stream_compute] + batch * blocksize * blocksize,
                blocksize,
                &beta,
                buf5_lesser_d + batch * blocksize * blocksize,
                blocksize));
            cublasErrchk(cublasZgemm(
                cublas_handle[stream_compute],
                CUBLAS_OP_N, CUBLAS_OP_N,
                ...
                buf2_d + batch * blocksize * blocksize,
                blocksize,
                greater_inv_diagblk_small_d[stream_compute] + batch * blocksize * blocksize,
                blocksize,
                &beta,
                buf5_greater_d + batch * blocksize * blocksize,
                blocksize));  
        }
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                buf3_lesser_d + batch * blocksize * blocksize,
                blocksize));
            cublasErrchk(cublasZgemm(
                cublas_handle[stream_compute],
                CUBLAS_OP_N, CUBLAS_OP_C,
                ...
                blocksize,
                &beta,
                buf3_greater_d + batch * blocksize * blocksize,
                blocksize));
        }
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgeam(
                ...
                blocksize, blocksize,
                &alpha,
                tmp_lesser_d + batch * blocksize * blocksize,
                blocksize,
                &beta,
                buf3_lesser_d + batch * blocksize * blocksize,
                blocksize,
            ...
            cublasErrchk(cublasZgeam(
                ...
                blocksize, blocksize,
                &alpha,
                tmp_greater_d + batch * blocksize * blocksize,
                blocksize,
                &beta,
                buf3_greater_d + batch * blocksize * blocksize,
                blocksize,
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                buf6_lesser_d + batch * blocksize * blocksize,
                blocksize));
            cublasErrchk(cublasZgemm(
                cublas_handle[stream_compute],
                CUBLAS_OP_N, CUBLAS_OP_N,
                ...
                blocksize,
                &beta,
                buf6_greater_d + batch * blocksize * blocksize,
                blocksize));
        }
        ...
        for(unsigned int batch = 0; batch < batch_size; batch++){
            cublasErrchk(cublasZgemm(
                ...
                blocksize,
                &beta,
                buf8_lesser_d + batch * blocksize * blocksize,
                blocksize));
            cublasErrchk(cublasZgemm(
                cublas_handle[stream_compute],
                CUBLAS_OP_N, CUBLAS_OP_N,
                ...
                blocksize,
                &beta,
                buf8_greater_d + batch * blocksize * blocksize,
                blocksize));
        }
