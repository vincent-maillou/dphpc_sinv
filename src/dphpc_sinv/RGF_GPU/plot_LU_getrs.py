...
if __name__ == "__main__":
    ...
    for j, blocksize in enumerate(block_sizes):
        ...
        for i, batchsize in enumerate(batchsizes):
            times_batched[j][i,:] = np.loadtxt(results_path + "times_solve_batched_"+ str(blocksize) + "_" + str(batchsize) + ".txt", dtype=np.float64)[warmup:]
    ...
    plt.savefig(images_path + "speedup_lu_getrs.png", bbox_inches='tight', dpi=300, format="png")
    plt.close(fig)
