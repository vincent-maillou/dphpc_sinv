...
if __name__ == "__main__":
    ...
    for j, blocksize in enumerate(block_sizes):
        for i, batchsize in enumerate(batchsizes):
            ...
            times_for[j][i,:] = np.loadtxt(results_path + "times_mm_for_"+ str(blocksize) +  "_" + str(batchsize) + ".txt", dtype=np.float64)[warmup:]

        median_for = np.median(times_for[j], axis=1)
        for i, batchsize in enumerate(batchsizes):
            times_batched[j][i,:] = np.loadtxt(results_path + "times_mm_batched_"+ str(blocksize) + "_" + str(batchsize) + ".txt", dtype=np.float64)[warmup:]
    ...
    # ax.set_yticks([1, 1.05, 1.10, 1.15], minor=False)
    # ax.get_yaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    # ax.get_yaxis().get_major_formatter().labelOnlyBase = False

    ax.set_yscale("log", base=2)
    # ax.set_yticks([0, 1, 5, 10, 15], minor=False)
    # ax.get_yaxis().set_major_formatter(matplotlib.ticker.ScalarFormatter())
    # ax.get_yaxis().get_major_formatter().labelOnlyBase = False
    ...
    ax.legend()
    plt.savefig(images_path + "speedup_mm_3.png", bbox_inches='tight', dpi=300, format="png")
    plt.close(fig)
