...
if __name__ == "__main__":
    ...
    for j, blocksize in enumerate(block_sizes):
        ...
        for i, batchsize in enumerate(batchsizes):
            times_batched[j][i,:] = np.loadtxt(results_path + "times_getrs_batched_"+ str(blocksize) + "_" + str(batchsize) + ".txt", dtype=np.float64)[warmup:]
    ...
    plt.savefig(images_path + "speedup_getrs_1.png", bbox_inches='tight', dpi=300, format="png")
    plt.close(fig)
