...
if __name__ == "__main__":
    ...
    for batchsize in batchsizes:
        for number_of_blocks in number_of_blockss:
            ...
            median_for = np.median(times_for, axis=1)
            for i, blocksize in enumerate(block_sizes):
                matrix_size = number_of_blocks * blocksize
                ...
                times_batched[i,:] = median_for[i]/times_batched[i,:]
                ...
                times_memcpy[i,:] = median_for[i]/times_memcpy[i,:]
            times = np.array([times_memcpy, times_batched])
            ...
            plt.savefig(images_path + "speedup_single_batch_lgr_"+ str(number_of_blocks) + "_"+ str(batchsize) + ".png", bbox_inches='tight', dpi=300, format="png")
            plt.close(fig)
