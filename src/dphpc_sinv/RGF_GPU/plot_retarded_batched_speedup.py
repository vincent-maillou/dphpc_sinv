...
if __name__ == "__main__":
    ...
    for number_of_blocks in number_of_blockss:
        flops_rgf = batchsize/1000*np.array([(number_of_blocks*flops_LU + 7*(number_of_blocks-1)*flops_gemm)*bs**3/512**3 for bs in block_sizes])
        ...
        ax.legend()
        plt.savefig(images_path + "scaling_batched_"+ str(number_of_blocks) + "_"+ str(batchsize)  +".eps", bbox_inches='tight', dpi=300, format="eps")
        plt.savefig(images_path + "scaling_batched_"+ str(number_of_blocks) + "_"+ str(batchsize) + ".png", bbox_inches='tight', dpi=300, format="png")
