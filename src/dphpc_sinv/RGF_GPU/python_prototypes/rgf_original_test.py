...
def test_rgf_lesser_greater_retarded_left_connected_tridiag_opt(
        matrix_size: int,
        blocksize: int
):
    rng = np.random.default_rng(seed=seed)
    ...
    reference_lesser_greater = reference_inverse @ B @ reference_inverse.T.conj()

    test_lesser_greater, test_inverse = rgf_original.rgf_lesser_greater_retarded_left_connected_tridiag_opt(A, B, blocksize)
    matrix_modification.cut_to_tridiag(reference_lesser_greater, blocksize)
    matrix_modification.cut_to_tridiag(reference_inverse, blocksize)

    matrix_modification.assert_block_tridiagonal(reference_lesser_greater, blocksize)
    matrix_modification.assert_block_tridiagonal(test_lesser_greater, blocksize)
    matrix_modification.assert_block_tridiagonal(reference_inverse, blocksize)
    matrix_modification.assert_block_tridiagonal(test_inverse, blocksize)

    assert np.allclose(test_lesser_greater[-blocksize:,-blocksize:], reference_lesser_greater[-blocksize:,-blocksize:])
    assert np.allclose(reference_lesser_greater, -reference_lesser_greater.conj().T)
    assert np.allclose(test_lesser_greater, -test_lesser_greater.conj().T)
    assert np.allclose(test_lesser_greater, reference_lesser_greater)
    assert np.allclose(test_inverse[-blocksize:,-blocksize:], reference_inverse[-blocksize:,-blocksize:])
    assert np.allclose(test_inverse, reference_inverse)
