    ...
    for(int k = 0; k < 1; k++){
        for(unsigned int batch = 0; batch < batch_size; batch++){
            ...
            */
            ...
            // printf("greater_diff_upperblk/greater_norm_upperblk = %e\n", greater9999999_diff_upperblk/greater_norm_upperblk);
        
            if(lesser_diff_diagblk/lesser_norm_diagblk > 1e-7 || lesser_diff_upperblk/lesser_norm_upperblk > 1e-7 || greater_diff_diagblk/greater_norm_diagblk > 1e-7 || greater_diff_upperblk/greater_norm_upperblk > 1e-7){
                not_failed = not_failed && false;
                std::cout << "FAILED not batched" << std::endl;
                printf("lesser_diff_diagblk/lesser_norm_diagblk = %e\n", lesser_diff_diagblk/lesser_norm_diagblk);
                printf("lesser_diff_upperblk/lesser_norm_upperblk = %e\n", lesser_diff_upperblk/lesser_norm_upperblk);
                printf("greater_diff_diagblk/greater_norm_diagblk = %e\n", greater_diff_diagblk/greater_norm_diagblk);
        ...
        // for(int i = 0; i < number_of_measurements; i++){
        //     time = -omp_get_wtime();
        //     rgf_lesser_greater_batched_without_hostmalloc(blocksize, matrix_size, batch_size,
        //                                 batch_system_matrices_diagblk_h,
        //                                 batch_system_matrices_upperblk_h,
        //                                 batch_system_matrices_lowerblk_h,
        //                                 batch_self_energy_matrices_lesser_diagblk_h,
        ...
        filename = "times_batched2_" + std::to_string(matrix_size) + "_"   + std::to_string(n_blocks) + "_"  + std::to_string(batch_size) + "_"+ ".txt";
        outputFile_times_batched2.open(filename);
        ...
        filename = "times_batched_optimized_" + std::to_string(matrix_size) + "_"   + std::to_string(n_blocks) + "_"  + std::to_string(batch_size) + "_"+ ".txt";
        outputFile_times_batched_optimized.open(filename);
        ...
        if(std::sqrt(diff_lesser_diagblk/norm_lesser_diagblk) > 1e-7 || std::sqrt(diff_lesser_upperblk/norm_lesser_upperblk) > 1e-7 || std::sqrt(diff_greater_diagblk/norm_greater_diagblk) > 1e-7 || std::sqrt(diff_greater_upperblk/norm_greater_upperblk) > 1e-7){
            not_failed = not_failed && false;
            std::cout << "FAILED Batched" << std::endl;
        }
